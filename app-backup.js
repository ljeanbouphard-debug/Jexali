...
let demoSales=Number(localStorage.getItem("jexaliSales")||0);
...
function renderDashboard(){
  ...
  document.getElementById("salesCount").textContent=demoSales;
...
document.getElementById("checkoutBtn").addEventListener("click",()=>{
  ...
  const subtotal=cart.reduce((s,p)=>s+Number(p.price),0);
  const fee=subtotal*.10;
  demoSales+=cart.length;
  jexaliRevenue+=fee;
  sellerRevenue+=subtotal-fee;
  localStorage.setItem("jexaliSales",demoSales);
  localStorage.setItem("jexaliRevenue",jexaliRevenue);
  localStorage.setItem("jexaliSellerRevenue",sellerRevenue);
  cart=[];
  localStorage.setItem("jexaliCart","[]");
  updateCartCount(); renderCart();
  alert("Demo order completed. No real payment was charged.");
});
