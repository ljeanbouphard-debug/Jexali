...
cart.length===0)
 ...
 const dbProducts = (await db.query('SELECT * FROM products WHERE id = ANY($1::int[])',[productIds])).rows; if (dbProducts.lentgh !== productsIds.length) return res.status(400).json({error:'Invalid product in cart'});
